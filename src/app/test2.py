...
########################################
# Helper Functions for Biomechanical Analysis (with Calibration)
########################################
...
def calculate_angle(a, b, c):
    try:
        ...
        logging.error("Error calculating angle: " + str(e))
        return 0

def calculate_biomechanics_mediapipe(points, calibration_factor=1.0):
    metrics = {}
    ...
    if points[11] and points[12]:
        metrics["shoulder_width"] = calibration_factor * np.linalg.norm(np.array(points[11]) - np.array(points[12]))
...
def calculate_biomechanics_movenet(points, calibration_factor=1.0):
    metrics = {}
    ...
    if points[5] and points[6]:
        metrics["shoulder_width"] = calibration_factor * np.linalg.norm(np.array(points[5]) - np.array(points[6]))
...
class OpenPoseAnalyzer:
    def __init__(self, model_path):
        try:
            ...
            logging.error("OpenPose loading error: " + str(e))
            self.net = None
        self.BODY_PARTS = {
    ...
    def detect_pose(self, frame, threshold=0.2):
        try:
            ...
            logging.error("OpenPose detect_pose error: " + str(e))
            return None
...
class MediaPipePoseAnalyzer:
    ...
    def calculate_body_metrics(self, points, calibration_factor=1.0):
        return calculate_biomechanics_mediapipe(points, calibration_factor)
...
class MoveNetAnalyzer:
    ...
    def __init__(self, model_path):
        try:
            ...
            logging.error("MoveNet loading error: " + str(e))
    ...
    def calculate_body_metrics(self, points, calibration_factor=1.0):
        if points is None:
            return {}
        return calculate_biomechanics_movenet(points, calibration_factor)
...
def run_video_estimation(analyzer, video_file, threshold, record_video=False, extract_skeleton=False, calibration_factor=1.0):
    ...
    chart_placeholder = st.empty()    # For realtime graphical metrics
    metrics_history = []
    progress_bar = st.progress(0)
    st.info("Processing video...")
    last_points = None
    frame_count = 0
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) if cap.get(cv2.CAP_PROP_FRAME_COUNT) > 0 else 100
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
        ...
        metrics = analyzer.calculate_body_metrics(points, calibration_factor)
        metrics_placeholder.markdown("**Realtime Metrics:** " + str(metrics))
        metrics_history.append(metrics.get("torso_alignment", 0))
        frame_count += 1
        progress_bar.progress(min(frame_count/total_frames, 1.0))
        if frame_count % 10 == 0:
            fig = px.line(x=list(range(len(metrics_history))), y=metrics_history, 
                          labels={"x": "Frame", "y": "Torso Alignment (°)"})
    ...
    if extract_skeleton and skeleton_frames:
        ...
        st.download_button(label="Download Skeleton Video", 
                           ...
                           mime="video/mp4")
    return rec_video_bytes, skel_video_bytes, analyzer.calculate_body_metrics(last_points, calibration_factor) if last_points is not None else {}
...
class ExerciseAnalysisTransformer(VideoTransformerBase):
    def __init__(self, analyzer, threshold, exercise_type="Squats"):
        self.analyzer = analyzer
        self.threshold = threshold
    ...
    def transform(self, frame):
        ...
        if self.exercise_type == "Squats":
            ...
            cv2.putText(img, "Exercise type not supported yet", (50,50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
        return img
...
def main():
    ...
    calibration_factor = st.sidebar.number_input("Calibration Factor", value=1.0, min_value=0.1, step=0.1)
    exercise_type = st.sidebar.selectbox("Exercise Type", ["Squats", "Other (Coming Soon)"])
    enable_multi = st.sidebar.checkbox("Enable Multi-Person Detection (Coming Soon)", value=False)
    
    MODEL_PATH = "src/graph_opt.pb"
    if model_choice == "MoveNet":
        moveNet_model_path = "src/movenet_lightning_fp16.tflite"
    ...
    if analysis_mode == "Video Pose Estimation":
        record_video = st.sidebar.checkbox("Record Processed Video", value=False)
        extract_skel_video = st.sidebar.checkbox("Extract Skeleton Video", value=False)
    ...
    **For Video Analysis:**  
    ...
    2. The video will be processed in real time with processed frames shown and metrics updated below.
       A realtime chart displays the torso alignment metric.
       You can also record the processed video and extract a skeleton video.
    ...
    **Session History:**  
    ...
    if analysis_mode == "Live Webcam Pose Detection":
        ...
        webrtc_streamer(key="exercise-analysis",
                        video_transformer_factory=lambda: ExerciseAnalysisTransformer(analyzer, threshold_val, exercise_type))
    # ----- Video Pose Estimation (File Upload) -----
        ...
        if video_file is not None:
            rec_vid, skel_vid, metrics = run_video_estimation(analyzer, video_file, threshold_val, record_video, extract_skeleton=extract_skel_video, calibration_factor=calibration_factor)
            if rec_vid:
                ...
                if points is not None:
                    ...
                    if analysis_mode == "Biomechanical Analysis":
                        ...
                        if model_choice == "MediaPipe":
                            metrics = calculate_biomechanics_mediapipe(points, calibration_factor)
                        elif model_choice == "MoveNet":
                            metrics = calculate_biomechanics_movenet(points, calibration_factor)
                        else:
